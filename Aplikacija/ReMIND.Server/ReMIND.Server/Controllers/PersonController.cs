        ...
        public async Task<ActionResult<Person>> Create([FromBody] Person newPerson) {
            try {                
                if (await _util.isCallerAdmin()) {
                    ...
                    newPerson.Password = _util.hashPassword(newPerson.Password);
                    newPerson.SessionID = "";
                    ...
                    //await _util.sendEmail(newPerson.Email, "Account creation", EmailInfo.setBody("Account", "Account created", _context.caller.Email, DateTime.Now.ToShortDateString(), $"<p>Your account has been created<br>Email: {newPerson.Email}<br>Password: {savePassword}<br>Please change the password once you log-in in the application.</p>"));
                    return Ok(newPerson);
                } else
